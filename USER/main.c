...
2026-4-26 01:05 Updated by LWH 添加sensor_led模块代码 实现人体感应功能 通过MQTT订阅云端命令控制RGB灯状态
2026-4-26 21:30 Updated by LWH 调整人体感应逻辑：无人时关闭RGB灯 有人时根据光照强度自动切换RGB灯状态（暗时白灯，亮时关闭）
2026-5-6  05:30 Updated by LWH 调整封装函数。整合各模块代码，完善注释说明，上传最终版本到GitHub
*/
    ...
    while (1)
    {
        ...
        if (MQTT_Init_Success)
        {
            ...
            if (DHT11_Read_TempAndHumidity(&DHT11_Data) && MQ135_Get_Air_Quality())
            {
                ...
                OLED_ShowNum(80, 20, Human_Read(), 1,OLED_8X16);
                ...
                if (Human_Read()==0)
                {
                    OLED_ShowString(20, 20, "0", OLED_8X16);
                    RGB_Off();
                }
                else
                {
                    OLED_ShowString(20, 20, "1", OLED_8X16);
                    if (LDR_ReadLight()<=45)
                    {
                     RGB_White();   /* code */
                    }
        ...
        uint8_t mode=MQTT_Get_Data("\"RGB\"","\"0\"", "\"1\""); //不断扫描云平台是否下发数据0
        ...
        /*char data1[10] = {0};
        char data2[10] = {0};
        uint8_t mode = MQTT_Get_Data("\"RGB\"", data1, data2);
        if (mode != 0xFF)
        {
            switch (mode)
            {
